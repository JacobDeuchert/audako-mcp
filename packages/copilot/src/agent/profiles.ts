...
export interface AgentProfile {
  ...
  callableAsSubagent: boolean;
}

const DEFAULT_MODEL: AgentModelSelection = {
  provider: appConfig.llm.provider as KnownProvider,
  modelName: appConfig.llm.modelName,
};

const PROFILE_REGISTRY: readonly AgentProfile[] = [
  {
    name: 'primary',
...
const PROFILES_BY_NAME = new Map<string, AgentProfile>();
...
export function listCallableProfiles(): AgentProfile[] {
  return PROFILE_REGISTRY.filter(profile => profile.callableAsSubagent).map(profile =>
...
export function resolveEffectiveTools(
  ...
  const uniqueRequested = new Set<string>();
