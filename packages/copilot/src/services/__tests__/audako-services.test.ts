...
describe('createAudakoServices', () => {
  ...
  it('should throw error if systemUrl is missing', async () => {
    await expect(createAudakoServices('', mockAccessToken)).rejects.toThrow(
      'systemUrl parameter is required',
    );
  });

  it('should throw error if accessToken is missing', async () => {
    await expect(createAudakoServices(mockSystemUrl, '')).rejects.toThrow(
      'accessToken parameter is required',
    );
  });
  ...
  it('should propagate errors from BaseHttpService.requestHttpConfig', async () => {
    ...
    await expect(createAudakoServices(mockSystemUrl, mockAccessToken)).rejects.toThrow(
      'Failed to fetch HTTP config',
    );
  });
});
