...
describe('SessionRegistry', () => {
  ...
  describe('getOrCreateSession', () => {
    it('creates a new session', async () => {
      ...
      const result = await registry.getOrCreateSession(
    ...
    it('reuses existing session for same credentials', async () => {
      ...
      const result1 = await registry.getOrCreateSession(
        'https://scada.example.com',
        'token123',
        createFn,
      );

      const result2 = await registry.getOrCreateSession(
        'https://scada.example.com',
        'token123',
        createFn,
      );
    ...
    it('creates separate sessions for different credentials', async () => {
      ...
      const result1 = await registry.getOrCreateSession(
        'https://scada1.example.com',
        'token123',
        createFn,
      );

      const result2 = await registry.getOrCreateSession(
        'https://scada2.example.com',
        'token456',
        createFn,
      );
  ...
  describe('verifySessionToken', () => {
    it('verifies valid token', async () => {
      ...
      const { entry, sessionToken } = await registry.getOrCreateSession(
        'https://scada.example.com',
        'token123',
        createFn,
      );
  ...
  describe('removeSession', () => {
    it('removes session and calls cleanup functions', async () => {
      ...
      const key = (registry as any).generateKey('https://scada.example.com', 'token123');
  ...
  describe('cleanupIdleSessions', () => {
    it('removes idle sessions', async () => {
      ...
      const { entry } = await shortRegistry.getOrCreateSession(
        'https://scada.example.com',
        'token123',
        createFn,
      );
  ...
  describe('cleanup task', () => {
    it('starts and stops cleanup task', () => {
      ...
      expect((registry as any).cleanupInterval).toBeDefined();

      registry.stopCleanupTask();
      expect((registry as any).cleanupInterval).toBeUndefined();
    });

    it('does not start multiple cleanup tasks', () => {
      registry.startCleanupTask(1000);
      const interval1 = (registry as any).cleanupInterval;

      registry.startCleanupTask(1000);
      const interval2 = (registry as any).cleanupInterval;
