import type { EntityTypeDefinition, SettingsTypeDefinition, TypeDefinition } from './types.js';
...
function getEntityTypeKeys(def: EntityTypeDefinition): string[] {
 ...
 */
export function registerSettingsType(def: SettingsTypeDefinition): void {
  const normalizedKey = normalizeTypeKey(def.key);

  if (typeRegistry.has(normalizedKey)) {
    throw new Error(
      `Settings type with key "${def.key}" is already registered. ` +
...
export function registerEntityType(def: EntityTypeDefinition): void {
  ...
  for (const key of entityKeys) {
    ...
    if (typeRegistry.has(normalizedKey)) {
      throw new Error(
        `Entity type key "${key}" (from entity "${def.key}") is already registered. ` +
          `Collision detected during registration.`,
      );
    }
  }
 ...
 */
export function resolveTypeDefinition(key: string): TypeDefinition | undefined {
  const normalizedKey = normalizeTypeKey(key);
 ...
 */
export function listAllTypeKeys(): string[] {
 ...
 */
export function getTypeRegistrySize(): number {
