...
interface SessionContextMock {
  getSessionId(): string;
  getGroupId(): string | undefined;
}

function getTool(name: string, tools: ReturnType<typeof createMutationTools>) {
...
describe('createMutationTools', () => {
  ...
  it('runs create-entity full pipeline in order and publishes event', async () => {
    const order: string[] = [];
    const context: SessionContextMock = {
      getSessionId: () => 'session-1',
      getGroupId: () => 'group-1',
    };
    ...
    const mutationThrottle = {
      execute: vi.fn(async (handler: () => Promise<unknown>) => {
        order.push('throttle');
        return handler();
      }),
    };
  ...
  it('stops before throttle when scope guard rejects', async () => {
    ...
    const throttle = {
      execute: vi.fn(async (handler: () => Promise<unknown>) => handler()),
    };
    const services = {
      entityData: {
        create: vi.fn(),
