...
import { SessionRequestCancelledError, SessionRequestTimeoutError, } from '../../services/session-request-hub.js';
...
describe('createAskQuestionTool', () => {
    it('submits question and returns formatted user response', async () => {
        const mockCreate = vi.fn(() => Promise.resolve(['Blue']));
    ...
    it('returns all selected options for multiple choice answers', async () => {
        const mockCreate = vi.fn(() => Promise.resolve(['Option A', 'Option C']));
        const mockToolRequestHub = {
            create: mockCreate,
            cancel: vi.fn(),
        };
    ...
    it('cancels pending request when execution is aborted', async () => {
        ...
        const mockCancel = vi.fn((sessionId) => {
        ...
        const execution = tool.execute('call-3', {
            question: 'Continue?',
            header: 'Confirm',
    ...
    it('propagates timeout errors from request hub', async () => {
        ...
        const mockCreate = vi.fn(() => Promise.reject(timeoutError));
        const mockToolRequestHub = {
            create: mockCreate,
            cancel: vi.fn(),
        };
        ...
        await expect(tool.execute('call-4', {
            ...
            multiple: false,
        }, new AbortController().signal)).rejects.toBe(timeoutError);
    });
});
//# sourceMappingURL=ask-question.test.js.map