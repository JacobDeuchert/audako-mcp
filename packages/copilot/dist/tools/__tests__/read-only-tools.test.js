...
function createMocks() {
    ...
    return {
        sessionContext,
        audakoServices,
        tools: createReadOnlyTools(sessionContext, audakoServices),
    };
}
...
describe('audako_mcp_get_entity_definition', () => {
    it('returns entity definition for supported entity type', async () => {
        ...
        const result = await tool.execute('call-1', { entityType: 'Signal' }, new AbortController().signal, vi.fn());
    ...
    it('returns MCP error payload for unsupported entity type', async () => {
        const { tools } = createMocks();
        const tool = getTool(tools, 'audako_mcp_get_entity_definition');
        const result = await tool.execute('call-1', { entityType: 'InvalidType' }, new AbortController().signal, vi.fn());
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toBe("Unsupported entity type 'InvalidType'.\n\n" +
...
describe('audako_mcp_get_entity_name', () => {
    it('returns entity display name for valid lookup', async () => {
        ...
        const result = await tool.execute('call-1', { entityType: 'Signal', entityId: 'sig-1' }, new AbortController().signal, vi.fn());
        expect(result.content[0].text).toBe('Main Pump');
    });
    it('returns error when entityId is empty', async () => {
        const { tools } = createMocks();
        const tool = getTool(tools, 'audako_mcp_get_entity_name');
        const result = await tool.execute('call-1', { entityType: 'Signal', entityId: '   ' }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
    ...
    it('returns unsupported entity type error with sorted supportedTypes', async () => {
        const { tools } = createMocks();
        const tool = getTool(tools, 'audako_mcp_get_entity_name');
        const result = await tool.execute('call-1', { entityType: 'BadType', entityId: 'id-1' }, new AbortController().signal, vi.fn());
        const supportedTypes = [...Object.values(EntityType)].sort((a, b) => a.localeCompare(b));
        expect(result.content[0].text).toBe(`Unsupported entity type 'BadType'.\n\n${JSON.stringify({ supportedTypes }, null, 2)}`);
        expect(result.isError).toBe(true);
    });
    it('returns service failure errors unchanged', async () => {
        ...
        const tool = getTool(tools, 'audako_mcp_get_entity_name');
        const result = await tool.execute('call-1', { entityType: 'Signal', entityId: 'sig-404' }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
...
describe('audako_mcp_get_group_path', () => {
    ...
    it('returns group breadcrumb payload exactly as MCP format', async () => {
        ...
        vi.spyOn(EntityNameService.prototype, 'resolvePathName').mockResolvedValue('Tenant One / Area A / Pump Group');
        const tool = getTool(tools, 'audako_mcp_get_group_path');
        const result = await tool.execute('call-1', { groupId: 'group-1' }, new AbortController().signal, vi.fn());
    ...
    it('returns error when groupId is empty', async () => {
        const { tools } = createMocks();
        const tool = getTool(tools, 'audako_mcp_get_group_path');
        const result = await tool.execute('call-1', { groupId: '   ' }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
    ...
    it('returns service failure errors unchanged', async () => {
        ...
        const tool = getTool(tools, 'audako_mcp_get_group_path');
        const result = await tool.execute('call-1', { groupId: 'group-404' }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
...
describe('audako_mcp_query_entities', () => {
    it('returns unsupported entity type error', async () => {
        ...
        const result = await tool.execute('call-1', { scope: 'global', entityType: 'Unknown', filter: {} }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
            content: [
                {
                    type: 'text',
                    text: "Unsupported entity type 'Unknown'.\n\n" +
                        JSON.stringify({ supportedTypes: ['Group', 'Signal'] }, null, 2),
                },
            ],
            isError: true,
    ...
    it('returns error for invalid filter object', async () => {
        const { tools } = createMocks();
        const tool = getTool(tools, 'audako_mcp_query_entities');
        const result = await tool.execute('call-1', { scope: 'global', entityType: 'Signal', filter: [] }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
    ...
    it('returns scope violation-style error when group scope id is unavailable', async () => {
        ...
        const tool = getTool(tools, 'audako_mcp_query_entities');
        const result = await tool.execute('call-1', { scope: 'group', entityType: 'Signal', filter: {} }, new AbortController().signal, vi.fn());
        expect(result.content[0].text).toBe("No 'group' scope ID provided and none found in session info.");
        expect(result.isError).toBe(true);
    });
    it('returns tenant root scope error for invalid tenant metadata', async () => {
        ...
        const tool = getTool(tools, 'audako_mcp_query_entities');
        const result = await tool.execute('call-1', { scope: 'tenant', entityType: 'Signal', filter: {} }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
            content: [
                {
                    type: 'text',
    ...
    it('queries by group scope and returns MCP payload with mapped entities', async () => {
        ...
        const tool = getTool(tools, 'audako_mcp_query_entities');
        const result = await tool.execute('call-1', { scope: 'group', entityType: 'Signal', filter: { Name: 'Pressure' } }, new AbortController().signal, vi.fn());
        expect(audakoServices.entityService.queryConfiguration).toHaveBeenCalledWith(EntityType.Signal, {
    ...
    it('returns query failure errors unchanged', async () => {
        const { tools, audakoServices } = createMocks();
        audakoServices.entityService.queryConfiguration.mockRejectedValue(new Error('Backend unavailable'));
        const tool = getTool(tools, 'audako_mcp_query_entities');
        const result = await tool.execute('call-1', { scope: 'global', entityType: 'Signal', filter: {} }, new AbortController().signal, vi.fn());
        expect(result).toEqual({
...
//# sourceMappingURL=read-only-tools.test.js.map