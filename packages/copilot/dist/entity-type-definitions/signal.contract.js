...
class SignalEntityContract extends ConfigurationEntityContract {
    ...
    assertNoFlatSettingsFields(payload) {
        const flatSettingsFields = Object.keys(payload).filter(key => !knownSignalBaseFields.has(key) && knownSignalSettingsFields.has(key));
        if (flatSettingsFields.length === 0) {
            return;
        }
        throw new Error(`Signal settings must be nested under 'settings'. Move these fields under settings: ${flatSettingsFields.join(', ')}.`);
    }
    extractPayloadSettings(payload) {
